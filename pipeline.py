...
LUA_SHA1 = get_hash(os.path.join(CWD, 'halo2.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            "--warc-header", "halo2-dld-script-version: " + VERSION,
            "--warc-header", ItemInterpolation("halo2-item: %(item_name)s"),
        ]
        ...
        if item_type == 'games':
            ...
            for i in range(int(start), int(end)+1):
                wget_args.append('http://halo.bungie.net/Stats/GameStatsHalo2.aspx?gameid=' + str(i))                
        else:
...
project = Project(
    title = "Halo 2",
    project_html = """
    <img class="project-logo" alt="logo" src="https://www.archiveteam.org/images/f/f0/Halo_2_Logo.png" />
...
pipeline = Pipeline(
    ...
    GetItemFromTracker("http://%s/%s" % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix="halo2"),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
